...
__all__ = [
    ...
    'md5_update_from_file', 'md5_update_from_dir', 'md5', 'download_url'
]
...
def md5_update_from_file(filename, hash_obj):
    """Updates the `hash_obj` with a given file.

    Args:
        filename: Path to the file.
        hash_obj: The `hashlib.md5` object to be updated.

    Returns:
        An updated `hashlib.md5` object.
    """
...
def md5_update_from_dir(directory, hash_obj):
    """Updates the `hash_obj` with a given directory.
    ...
    for path in sorted(Path(directory).iterdir()):
        ...
        if path.is_file():
            hash_obj = md5_update_from_file(path, hash_obj)
        elif path.is_dir():
            hash_obj = md5_update_from_dir(path, hash_obj)
    return hash_obj


def md5(path):
    """Returns the MD5 of the given path in hex digest."""
    if Path(path).is_file():
        md5_hash = md5_update_from_file(path, hashlib.md5())
        return md5_hash.hexdigest()
    if Path(path).is_dir():
        md5_hash = md5_update_from_dir(path, hashlib.md5())
        return md5_hash.hexdigest()
    raise ValueError(f'Currently calculation of MD5 does not supported for '
                     f'`{path}`')
